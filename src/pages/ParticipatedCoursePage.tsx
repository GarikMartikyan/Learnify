...
export default function ParticipatedCoursePage() {
  // Determine if an exam should be locked
            ...
            return (
              ...
              >
                <List.Item.Meta
                  ...
                  description={
                    item.type === "chapter" ? (
                      <Tag color="blue">Chapter</Tag>
                    ) : item.passed ? null : locked ? (
                      <Tag color="red">Locked</Tag>
                    ) : (
                      <Tag color="orange">Exam</Tag>
                    )
                  }
                />
                ...
                {item.type === "chapter" && (
                  <Link to={routes.chapterById("1")}>
                    <Button type="primary">Open Chapter</Button>
                  </Link>
                )}

                {item.type === "exam" && !locked && !item.passed && (
                  <Button type="primary">Take Exam</Button>
                )}
              </List.Item>
            );
          }}
